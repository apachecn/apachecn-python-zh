...
![Figure 10.1: Slope of a line ](image/B15968_10_01.jpg)
...
![Figure 10.2: Finding the slope of a curve ](image/B15968_10_02.jpg)

图 10.2：寻找曲线的斜率

然而，如果我们将点**A**放大得足够近，我们会看到曲线非常接近于一条直线。

![Figure 10.3: Zooming in on the curve ](image/B15968_10_03.jpg)
...
![Figure 10.4: Formula for calculating derivatives ](image/B15968_10_04.jpg)
...
![Figure 10.5: Function definitions at given x values ](image/B15968_10_05.jpg)
...
4.  Then print out the derivatives by calling each function and the desired *x* value:
    ...
    ![Figure 10.6: Two tangent lines to a curve ](image/B15968_10_06.jpg)

    图 10.6：曲线的两条切线

    让我们使用*图 10.6*中的信息。方程式如下：

    ![Figure 10.7: Equation of f(x) ](image/B15968_10_07.jpg)
    ...
    *图 10.6*中**A**点的*x*值为-0.48，**B**点的*x*值为 0.67。使用 Python 实现这一点的好处是，不管给定的值是整数、负数还是小数，计算机都可以轻松处理数字运算。
    ...
    ![Figure 10.8: Equation of a line ](image/B15968_10_08.jpg)

    图 10.8：直线方程

    我们给出了函数和点（*x*0*，y*0），因此，我们可以从函数在给定*x*值的导数中找到斜率*m*。切线的方程式为*y=mx+b*，我们唯一不知道的是*b*，即直线的*y*截距。但是如果我们重新排列前面的方程，我们可以在方程的右边看到它：

    ![Figure 10.9: Equation of line at the point ](image/B15968_10_09.jpg)
...
10.  Now, to get the equations of the lines tangent to `f` at *x = -0.48* and *x = 0.67*, use the following code:
    ...
    *图 10.10*显示了一个函数及其下的区域。通常，该区域由函数本身、较低的*x*值**a**、较高的*x*值**b**和*x*轴限定。
    ...
    ![Figure 10.10: The area S under a curve defined by the function f(x) from a to b ](image/B15968_10_10.jpg)

    图 10.10：由函数 f（x）从 a 到 b 定义的曲线下的面积 S

    我们要做的是将区域**S**切成等宽的矩形，因为我们知道高度（*f（x）*，所以使用 Python 很容易将它们相加。*图 10.11*显示了*f（x）=x*2 的情况：

    ![Figure 10.11: The area S sliced into 10 rectangles of equal width ](image/B15968_10_11.jpg)
    ...
    ![Figure 10.12: Smaller rectangles making a better approximation of the area ](image/B15968_10_12.jpg)
    ...
    ![Figure 10.13: Using trapezoids for better approximations to the curve ](image/B15968_10_13.jpg)

    图 10.13：使用梯形更好地逼近曲线

    以下是梯形规则的公式：

    ![Figure 10.14: Formula for area of trapezoids ](image/B15968_10_14.jpg)
    ...
    ![Figure 10.15: Formula for intervals ](image/B15968_10_15.jpg)
...
![Figure 10.16: A parabola rotated about the z axis ](image/B15968_10_16.jpg)
...
![Figure 10.17: Flipping the paraboloid on its side ](image/B15968_10_17.jpg)
...
![Figure 10.18: A two-dimensional look at the two functions ](image/B15968_10_18.jpg)

图 10.18：两个功能的二维视图

固体的最终形状如下所示：

![Figure 10.19: The resulting shape is like a ring ](image/B15968_10_19.jpg)

图 10.19：生成的形状类似于一个环

如上图所示，这就像一个求环面积的问题。公式如下：

![Figure 10.20: Formula for area of a ring ](image/B15968_10_20.jpg)
...
![Figure 10.21: A cubic function and the points we want to find ](image/B15968_10_21.jpg)
...
![Figure 10.22: Distance of the lighthouse from the cabin ](image/B15968_10_22.jpg)
...
1.  We're aiming to minimize the time it takes to make this trip, so let's make a formula for time. Remember, time is distance divided by rate:
    ...
    ![Figure 10.23: Formula for calculating time ](image/B15968_10_23.jpg)
...
![Figure 10.24: Cutting squares out of the corners of a rectangle ](image/B15968_10_24.jpg)
...
1.  The formula for the volume of the box will be length multiplied by width multiplied by height. In terms of *x*, the length of the square cut from the corners, the length of the box is *12 – 2x*, since two corners are cut out of the 12-inch sides. Similarly, the width of the box will be *10 – 2x*. The height, once the "flaps" are bent upwards, will be *x*. So, the volume is:
    ...
    ![Figure 10.25: Formula to calculate the volume ](image/B15968_10_25.jpg)
...
![Figure 10.26: Plot of maximum value achieved ](image/B15968_10_26.jpg)
...
1.  The surface area of a cylinder is the area of the bottom (a circle, so πr2) plus the area of its side, which is a rectangle of base *2πr* and a height of *h*. The volume of a cylinder is πr2h, so we put it all together:
    ...
    ![Figure 10.27: Formula to calculate the volume of a cylinder ](image/B15968_10_27.jpg)
...
2.  The volume is already set to 355\. From there, we can get an expression for *h* in terms of *r* and we'll have the surface area all in terms of one variable:
    ...
    ![Figure 10.28: Substituting the values in the formula ](image/B15968_10_28.jpg)
...
![Figure 10.29: Finding the minimum material needed to make a can ](image/B15968_10_29.jpg)
...
![Figure 10.30: Ships A and B moving south and east ](image/B15968_10_30.jpg)
...
1.  The distance is velocity multiplied by time, so the distance between the two ships can be modeled by this equation:
    ...
    ![Figure 10.31: Formula for calculating distance ](image/B15968_10_31.jpg)
...
![Figure 10.32: Plot of distance versus time ](image/B15968_10_32.jpg)
...
![Figure 10.33: Circle to cone volume ](image/B15968_10_33.jpg)
