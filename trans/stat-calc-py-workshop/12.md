...
在数学课上解一个方程通常涉及一个未知的数字，*x*。该公式隐藏了该值，但为您提供了如何找到该值的提示，例如![1](image/B15968_12_InlineEquation1.png)。但是要解一个微分方程，你只需要得到关于函数导数的信息，你需要找到函数。可以是以下简单的事情：

![Figure 12.1: Finding a function with derivative 2 ](image/B15968_12_01.jpg)

图 12.1：寻找具有导数 2 的函数

这意味着*找到一个导数为 2 的函数。*也可以这样写：

![Figure 12.2: Alternative way to represent derivative of the function ](image/B15968_12_02.jpg)
...
![Figure 12.3: Derivative of a function whose value is the function itself ](image/B15968_12_03.jpg)
...
![Figure 12.4: Formula for simple interest ](image/B15968_12_04.jpg)
...
2.  As you can see in the docstring of the `amount` function, it will take a starting amount and a rate of growth and return the amount of the investment after the given number of years. Let's see the interest earned in 1-5 years:
    ...
    ![Figure 12.5: Formula to calculate the amount after t years ](image/B15968_12_05.jpg)
...
1.  To approximate compounding continuously, we'll compound the interest every second (*365*24*60*60* times per year):
    ...
    ![Figure 12.6: Formula to calculate compound interest ](image/B15968_12_06.jpg)
...
![Figure 12.7: Differential equation to calculate population at time t ](image/B15968_12_07.jpg)

图 12.7：计算 t 时总体的微分方程

这个微分方程意味着*y*的变化率与*y*成正比，或者种群增长与其数量成正比。这就是人口增长率的定义：人口的一小部分或百分比。解决方案类似于我们涉及连续复利的利息问题：

![Figure 12.8: Differential equation to calculate the rate of change ](image/B15968_12_08.jpg)
...
2.  We're looking for the time, *t*, that will, with a growth rate of 4%, turn our initial population of 1 into 2\. We estimate it will be somewhere between 1 and 100 years:
    ...
    ![Figure 12.9: Equation to solve for time (t) ](image/B15968_12_09.jpg)
...
![Figure 12.10: Differential equation for calculating rate of decay of a substance ](image/B15968_12_10.jpg)

图 12.10：计算物质衰变率的微分方程

该解决方案与我们的人口问题类似，只是衰减因子为负，因为数量减少：

![Figure 12.11: Calculating rate of change with negative decay factor ](image/B15968_12_11.jpg)
...
![Figure 12.12: The Pert formula ](image/B15968_12_12.jpg)
...
2.  This is the line that's changed. We put a beginning amount of `1` into the `pert` function, *r* will be `avg`, and `5730` will be the target time:
    ...
    *r=-0.000120968*，因此我们的 Pert 公式如下：
    ...
    ![Figure 12.13: Substituting the new value of r in the Pert formula ](image/B15968_12_13.jpg)

    图 12.13：在 Pert 公式中替换新的 r 值

    这意味着*x*克碳-14 已经腐烂，剩下 10 x 克，是原来的 10 倍。因此，衰减量为整个样品的 1/11 或 0.091。期末金额为 1–0.091。这使得我们的 Pert 方程如下：

    ![Figure 12.14: Pert equation with ending amount ](image/B15968_12_14.jpg)
...
![Figure 12.15: Differential equation for rate of change of temperature ](image/B15968_12_15.jpg)
...
![Figure 12.16: Equation for calculating the time of death ](image/B15968_12_16.jpg)
...
3.  Here's the important change: our original amount (the temperature difference) is 15 degrees, and we want to know *r*, the rate of change in our Pert formula:
    ...
    ![Figure 12.17: A graph of the cooling body ](image/B15968_12_17.jpg)

    图 12.17：冷却体图

    我们所需要知道的是，这个差值衰减到 10 所需的小时数。我们的方程式如下：

    ![Figure 12.18: Number of hours taken for the difference to decay to 10 ](image/B15968_12_18.jpg)
...
1.  The difference starts at 103° (175-72). In 0.25 hours, it's changed to 68° (140-72). Now, we can set up an equation:
    ...
    ![Figure 12.19: Equation for calculating difference in the coffee's temperature ](image/B15968_12_19.jpg)
...
3.  Now, 2 gallons of the brine flows out every minute, but how much salt is in it? That requires us to find the concentration of each gallon of brine:
    ...
    ![Figure 12.20: Formula to calculate the concentration of each gallon of brine ](image/B15968_12_20.jpg)
...
你得到了微分方程![2](image/B15968_12_InlineEquation4.png)。您想知道函数*y=f（x）*在特定值*x*下的输出。图中给出了一点：*f（0）=1*。这意味着，*“该函数在每个点上的导数是该点上函数的 y 值。”*记住，导数是曲线图上点的斜率或方向。Euler 的方法是从初始值开始，在这种情况下，从（0,1）开始，然后使用微分方程计算到下一点的方向。**微分方程****DE**表示斜率为*y*-值，所以我们在*x*正方向上迈出了一小步：

![Figure 12.21: Taking a small step (hopefully) in the right direction ](image/B15968_12_21.jpg)

图 12.21：朝着正确的方向迈出一小步（希望如此）

衍生工具如下：

![Figure 12.22: Derivative of the function ](image/B15968_12_22.jpg)

图 12.22：函数的导数

因此，*ΔY*变为：

![Figure 12.23: Formula to calculate ΔY ](image/B15968_12_23.jpg)
...
![Figure 12.24: Better approximations with a smaller stepsize ](image/B15968_12_24.jpg)

图 12.24：较小步长的更好近似值

第四条曲线（右边的曲线）是步长为 1 的近似值的路径。第三条曲线的步长为½，第二条曲线为¼，第一条曲线为 1/8。我们选择![a](image/B15968_12_InlineEquation2.png)微分方程，因为我们知道代数解。

当*x*为 2 时，*e*2*=7.389*。加上*y=e*x 的实际曲线（左边的第一条曲线），我们可以看到步长越小，近似值越接近实际曲线：

![Figure 12.25: The actual curve added to the left side of the first curve ](image/B15968_12_25.jpg)
...
![Figure 12.26: Euler's method on initial VP ](image/B15968_12_26.jpg)
...
![Figure 12.27: Equations when f(x,y) is given ](image/B15968_12_27.jpg)

图 12.27：给出 f（x，y）时的方程

在接下来的*y*中，我们将前面的四个近似值进行平均，并将*k*2 和*k*3 的权重增加一倍：

![Figure 12.28: Formula for averaging the 4 preceding approximations ](image/B15968_12_28.jpg)

图 12.28：上述 4 种近似值的平均公式

然后，当然，*x*增加*h*：

![Figure 12.29: Incrementing x by h ](image/B15968_12_29.jpg)
...
![Figure 12.30: Runge Kutta method with stepsize 0.2  ](image/B15968_12_30.jpg)
...
3.  The key is that the change in *y* over the change in *x* between the locations of the prey and the predator represents the tangent of the angle we want. We know their locations, so we use the inverse tangent function, called `atan2`, to calculate the angle so that the predator points directly at the prey. All we really want is to know how much to change the predator's *x*- and *y*-coordinates for them to move 1 unit toward the prey. To turn the predator toward the prey, we need to find the angle between the two points, as shown in the following diagram:
    ...
    ![Figure 12.31: The angle between predator and prey ](image/B15968_12_31.jpg)
...
7.  If you run this, you can watch the chase. Here's what the final output should look like:
    ...
    ![Figure 12.32: The path of the predator is a logarithmic curve ](image/B15968_12_32.jpg)
...
8.  **Extension**: Change the prey's path into a circle. After the line to make the prey move forward, add this line:
    ...
    ![Figure 12.33: The pursuit curve when the prey is fleeing in a circular path ](image/B15968_12_33.jpg)
...
1.  In algebra class, we're led to calculate the height of a projectile using an equation:
    ...
    ![Figure 12.34: Formula to calculate the height of a projectile ](image/B15968_12_34.jpg)
...
4.  The velocity and height are assigned their starting values, *v*0 and *h*0, and then the velocity is updated by *g* and the acceleration (due to gravity), and then the height, *h*, is updated by the velocity. We repeat our calculation every second and check to see when the ball's height returns to zero:
    ...
    ![Figure 12.35: Substituting the values in the formula for calculating the height of a projectile ](image/B15968_12_35.jpg)
...
![Figure 12.36: The path of a projectile without air resistance ](image/B15968_12_36.jpg)
...
![Figure 12.37: The height of a projectile, with air resistance (the inner curve) ](image/B15968_12_37.jpg)
...
![Figure 12.38: The velocity of a body in free fall with air resistance ](image/B15968_12_38.jpg)
...
![Figure 12.39: Differential equation for the velocity vector of a particle ](image/B15968_12_39.jpg)
