...
数值*c<sub>i</sub>*与频率*f<sub>i</sub>*配对。这对![Getting ready](Image00030.jpg)和![Getting ready](Image00031.jpg)进行了两个小改动，以执行类似的计算：

![Getting ready](Image00032.jpg)

![Getting ready](Image00033.jpg)

我们定义了![Getting ready](Image00030.jpg)使用频率和值的乘积。同样，我们定义了![Getting ready](Image00031.jpg)来使用频率。我们在每个名称上添加了 hat，^，以明确这些函数不适用于简单的值列表；这些函数用于（值、频率）对的列表。
...
我们已经将![How it works...](Image00030.jpg)和![How it works...](Image00031.jpg)的每个定义转换为生成器表达式。由于 Python 的设计目的是严格遵循数学形式，因此代码以相对直接的方式遵循数学。

## 还有更多。。。

为了计算方差（和标准差），我们需要在这个主题上再增加两个变量。我们可以定义频率分布的总体平均值μ<sub>*F*</sub>：

![There's more...](Image00034.jpg)

其中*c<sub>i</sub>*是来自`Counter`对象的密钥，*F*，*F<sub>i</sub>*是来自`Counter`对象的给定密钥的频率值。

方差 VAR<sub>*F*</sub>可以根据均值μ<sub>*F*</sub>来定义。公式如下：

![There's more...](Image00035.jpg)

这计算值*c*<sub>*i*</sub>与平均值μ<sub>*F*</sub>之间的差值。该值通过该值出现的次数*f<sub>i</sub>*进行加权。这些加权差值之和除以计数![There's more...](Image00031.jpg)，减去一。
...
![There's more...](Image00036.jpg)

![There's more...](Image00037.jpg)

这需要一个额外的求和计算。我们需要计算值的平方和，![There's more...](Image00038.jpg)：
...
考虑到这三个求和函数![There's more...](Image00031.jpg)、![There's more...](Image00030.jpg)和![There's more...](Image00039.jpg)，我们可以为组合汇总*F*定义方差：
...
1.  确定所需的各种金额。对于这个表达式，我们可以看到以下内容：计数*n*实际上可以定义为源数据集中每个数据的 1 之和。这也可以被认为是*x<sub>i</sub>*<sup>∘</sup> 或*y<sub>i</sub>*<sup>∘</sup> 。
    ...
    *   ![How to do it...](Image00041.jpg)The count,
2.  从`math`模块
...
最终的相关比可以稍微简化。当我们将更复杂的外观![How it works...](Image00042.jpg)替换为略具蟒蛇特征的*S*（*x*时，我们可以更好地看到方程的整体形式：
...
![Getting ready](Image00044.jpg)

给定自变量*x*，根据α和β参数计算因变量![Getting ready](Image00045.jpg)的估计值或预测值。

目标是找到α和β的值，这些值在估计值![Getting ready](Image00045.jpg)和*y*的实际值之间产生最小的总体误差。以下是β的计算：
...
在所有情况下，方程式均为![How to do it...](Image00046.jpg)。这个估计似乎是对*y*实际值的一个很好的预测。
...
![Long-term model](Image00048.jpg)
...
![There's more...](Image00052.jpg)

为了确定每个*x<sub>i</sub>*值的异常值状态，必须计算该异常值。该决策函数需要两个额外的输入：人口中位数![There's more...](Image00053.jpg)和 MAD 值。这使得过滤器决策功能相当复杂。它看起来是这样的：
