...
![Getting ready](Image00013.jpg)

这里，*k*是来自`Counter`、*C*的密钥，*f<sub>k</sub>*是来自`Counter`的给定密钥的频率值。

标准偏差*σ*取决于平均值*μ*。这还涉及到计算值的总和，每个值都按频率加权。公式如下：

![Getting ready](Image00014.jpg)

这里，*k*是来自`Counter`、*C*的键，*f<sub>k</sub>*是来自`Counter`的给定键的频率值。`Counter`中的项目总数为![Getting ready](Image00015.jpg)。这是频率的总和。
...
这里，*k*是来自`Counter`、*C*的密钥，*f<sub>k</sub>*是来自`Counter`的给定密钥的频率值。柜台内物品总数为![Getting ready...](Image00015.jpg)。
