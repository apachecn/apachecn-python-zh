...
*   我们用单回勾（```号）环绕引用。链接后面跟着一个`_`。我们可以使用``section title`_`来引用文档中的特定部分。我们通常不需要在 URL 周围放置任何内容。Docutils 工具可以识别这些。有时我们希望显示一个单词或短语，并隐藏 URL。我们可以使用这个：``the Sphinx documentation <http://www.sphinx-doc.org/en/stable/>`_`。
*   我们可以用双回勾（````）环绕代码相关的单词，使它们看起来像```code```。

还有一种更通用的技术，称为文本角色。角色看起来比简单地用`*`字符包装一个单词或短语要复杂一些。我们使用`:word:`作为角色名称，后跟单个```回勾中的适用单词或短语。文本角色类似于`:strong:`this``。
...
我们可以使用![Designing complex if...elif chains](Image00003.jpg)来表示与`any(C)`或`any([C_1, C_2, C_3, ..., C_n])`类似的意思。我们需要证明![Designing complex if...elif chains](Image00004.jpg)；我们不能假设这是`true`。
...
*   我们可能有更复杂的事情。既然我们知道整个宇宙，我们就可以证明![How to do it...](Image00004.jpg)。我们需要使用这个方法来编写一系列的`if`和`elif`语句，每个条件一个子句。
